...
When(/^I wait until "([^"]*)" is( not)? (displayed|clickable)$/, async function (element, notArg, condition) {
	this.info(`i wait until '${element}' is ${notArg ? "not " : ""}displayed`);
	switch (condition) {
	case "displayed":
		await waitForChildBeingDisplayed(element, {reverse: Boolean(notArg), timeout: 20000});
		break;
	case "clickable":
		await waitForChildBeingClickable(element,{reverse: Boolean(notArg)});
	}
});
